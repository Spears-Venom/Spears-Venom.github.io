...
function circleHover(chosen) {
	...
	if (hoverType == "city") {d3.select("#callOutCity").html(chosen.city + " urban area, " + chosen.State);}
	if (hoverType == "country") {d3.select("#callOutCity").html(chosen.State);}
	d3.select("#td-pop-2000").html(numFormatTwo(chosen.pop_2001/1e6));
	d3.select("#td-land-2000").html(chosen.land_2001);
	d3.select("#td-density-2000").html(Math.round(chosen.density_2001));
	d3.select("#td-pop-2010").html(numFormatTwo(chosen.pop_2011/1e6) + ' <span style="font-size: 9px;">(' + chosen.rank_pop + ')</span>');
	d3.select("#td-land-2010").html(chosen.land_2011 + ' <span style="font-size: 9px;">(' + chosen.rank_land + ')</span>');
	d3.select("#td-density-2010").html(Math.round(chosen.density_2011) + ' <span style="font-size: 9px;">(' + chosen.rank_density + ')</span>');
	d3.select("#td-pop-perc").html(numFormatOne((chosen.pop_2011-chosen.pop_2001)/chosen.pop_2001*100) + "%");
	d3.select("#td-land-perc").html(numFormatOne((chosen.land_2011-chosen.land_2001)/chosen.land_2001*100) + "%");
	d3.select("#td-density-perc").html(numFormatOne((chosen.density_2011-chosen.density_2001)/chosen.density_2001*100) + "%");
...
function highlight(d) {
	...
	cities2000.selectAll(".city_2000")
		.filter(function(d) {return d.State != chosen.State;})
		.style("fill-opacity", setOpacity);
