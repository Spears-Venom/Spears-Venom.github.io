...
function introDotHistogram() {
	...
	d3.select("#explanation")
		...
		.call(endall, function() {
				d3.select("#explanation")
					...
					.style("left", 600 + "px")
					.style("width", 350 + "px")
					.html("<p>Looking at the urban population growth we find that many cities have seen an enormous growth in their population between <span style='color: #858483;'>2001</span> and <span style='color: #DA6761;'>2011</span></p>");	
		})
		.transition().duration(1000)
		.style("opacity",1);
	...
	*/
	...
	explanationText(varText = "<p>The urban population density has seen both large increase and decline</p>" + 
							  "<p>However, 94% of the cities that saw a density decline are in West Bengal. This could be caused by the phenomenon of 'Ghost cities' and rising incomes</p> ", 
	...
	setTimeout(function () {
		cities2010.selectAll(".city_2010")
			...
			.style("fill-opacity", function(d) {
				if (d.State == "West Bengal" & d.density_growth < 0) return 1;
				else return 0.2;
			});
	}, delay*2.5)
	...
	explanationText(varText = "Practically the entire area of highest density growth comes from low & low-middle income States with populations that usually migrate freely", 
		...
		d3.select("#section")
			...
			.call(endall, function() {
				...
				d3.select("#sectionText")
					.html('<p>These plots show the rapid growth in land and population within the cities. Growth may lead to numerous economic opportunities. However, it also forces urban policy makers and planners to rethink the design of their cities. The arrangement of future infrastructure can already be a crucial decision. Each city has its own challenges and opportunities and each State its own way of growing</p>' + 
