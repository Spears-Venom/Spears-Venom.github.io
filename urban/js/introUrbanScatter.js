...
function introUrbanScatter() {
	...
	d3.select("#explanation")
		...
		.call(endall, function() {
				d3.select("#explanation")
					...
					.style("left", 590 + "px")
					.style("width", 380 + "px")
					.html("Finally, let's zoom into the States and look at what the future might hold");	
		})
	...
	barChart.selectAll(".city_2010")
		.transition().duration(2000).delay(2000)	
		.attr("width", function(d) {return barScale(eval("d." + rVar + "_2011"));});
	
	//Update the 2000 bar widths
	barChart.selectAll(".city_2000")
		.transition().duration(2000).delay(2000)	
		.attr("width", function(d) {return barScale(eval("d." + rVar + "_2001"));});
	...
	setTimeout(function() {	
		...
		barChart.selectAll(".barWrapper")
			.filter(function(d) {return d.State == "Kerala";})
			.transition().duration(1000)
			.style("opacity", 0)
			.remove();
		...
		barChart.selectAll(".city_2000")
			...
			.attr("transform", function (d) {
				return "translate(" + (barScale(d.pop_perc_2001) - dotSize/2) + "," + (barHeight/2 - dotSize/2) + ")";
			});
		...
		barChart.selectAll(".city_2010")
			...
			.attr("height",dotSize)
			.attr("transform", function (d) {
				return "translate(" + (barScale(d.pop_perc_2011) - dotSize/2) + "," + (barHeight/2 - dotSize/2) + ")";
			});
			
		//Create the lines between the 2000 and 2010 dots
		barChart.selectAll("line")
			.attr("x1", function(d) {return barScale(d.pop_perc_2001);})
			.attr("x2", function(d) {return barScale(d.pop_perc_2011);})
			.attr("y1", (barHeight/2))
			.attr("y2", (barHeight/2))
		...
		explanationText(varText = "<p>Even with the enormous shift of 40 million people moving towards cities within one decade, most States still have far less than 50% of their total population living in a city</p>" +
	...
	setTimeout(function() {	
		...
		explanationText(varText = "<p>While most States have seen a more free migration towards cities, their contribution to GDP has grown between <span style='color: #858483;'>2001</span> and <span style='color: #DA6761;'>2011</span>.</p>" + 
		...
		barChart.selectAll(".barWrapper")
			.filter(function(d) {return d.gdp_2001 == 0;})
			.transition().duration(1000)
			.style("opacity", 0)
			.remove();
		...
		d3.select("#section")
			...
			.call(endall, function() {
				...
				d3.select("#sectionText")
					.html('<p>Even though it would almost appear as if all inhabitants of India live in cities, the large majority of the citizens currently still live in rural areas. Continuous urban growth is therefore expected to occur in the coming decades</p>' +
						  '<p>The current view reveals a correlation between a rise in the urban population as a percentage to the whole population and an increase in the contribution to GDP (for the States where Domestic Product information is available) showing a glance of the possible economic prosperity that continuous urban growth might bring to an urban area and people that might move here in the coming future</p>' +
						  ...
						  '<p><span style="color: #A8A8A8;">To see which State belongs to each circle-line element hover over the plot on the left</span></p>');		
			})
			.transition().duration(1000)
			.style("opacity",1);
