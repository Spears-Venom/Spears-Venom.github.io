	...
	d3.select("#section")
		...
		.call(endall, function() {
			...
			d3.select("#sectionText")
				.html('<p>Urban population density, which was already high in the year 2001 for India has only grown larger for practically all cities. Continuous growth can be expected in the coming years. Therefore, the focus for urban planners should be on the design of this density, the appropriate coordination and location of different city aspects</p>' + 
...
function changeSlope() {	
	...
	slopeScale.domain([d3.min(data, function(d) {return eval("d." + rVar + "_2001");}),d3.max(data, function(d) {return eval("d." + rVar + "_2011");})]);
	...
	cities2000.selectAll(".city_2000")
		...
		.attr("cy", function(d){return slopeScale(eval("d." + rVar + "_2001"));});
	...
	cities2010.selectAll(".city_2010")
		...
		.attr("cy", function(d){return slopeScale(eval("d." + rVar + "_2011"));});
	...
	slopes.selectAll(".slopes")
		...
		.call(endall, function () {
			slopes.selectAll(".slopes")
				...
				.attr("y1",function(d){return slopeScale(eval("d." + rVar + "_2001"));})
				.attr("y2",function(d){return slopeScale(eval("d." + rVar + "_2011"));})
				.transition().duration(dur)
				.style("opacity", 0.4)
		});
	...
	text2000.selectAll("text")
		...
		.call(endall,  function() {
			text2000.selectAll("text")
				...
				.attr("y", function(d){return slopeScale(eval("d." + rVar + "_2001"));})
				.text(function(d) {
					if (rVar == "pop") {
						if (d.pop_2001 > 1e6) {return d.city + ", " + d.State + " | " + numFormatOne(d.pop_2001/1e6);} 
						else {return d.city + ", " + d.State + " | " + numFormatTwo(d.pop_2001/1e6);}
					} else if (rVar == "land") {
						return d.city + ", " + d.State + " | " + d.land_2011;
					} else if (rVar == "density") {
						return d.city + ", " + d.State + " | " + numFormatTwo(d.density_2001/1e3); 
					}//else if
				})				
	...
	text2010.selectAll("text")
		...
		.call(endall,  function() {
			text2010.selectAll("text")
				...
				.attr("y", function(d){return slopeScale(eval("d." + rVar + "_2011"));})
				.text(function(d) {
					if (rVar == "pop") {
						if (d.pop_2001 > 1e6) {return numFormatOne(d.pop_2011/1e6) + " | " + d.city + ", " + d.State;} 
						else {return numFormatTwo(d.pop_2011/1e6) + " | " + d.city + ", " + d.State; }
					} else if (rVar == "land") {
						return d.land_2011 + " | " + d.city + ", " + d.State;
					} else if (rVar == "density") {
						return numFormatTwo(d.density_2011/1e3) + " | " + d.city + ", " + d.State; 
					}//else if
				})
				.filter(function(d) { return eval("d.rank_" + rVar + " < 10");})
				.transition().duration(dur/2)			
