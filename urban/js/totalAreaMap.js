	...
	d3.select("#section")
		...
		.call(endall, function() {
			...
			d3.select("#sectionText")
				.html('<p>The urban areas in this study have 100,000 or more inhabitants in 2011. In 2001 there were already 120 million people living in these 54 cities and just one decade on this number grew to 160 million. That\'s 40 million people more!<br>With this amount of people on the move, it becomes imperative for cities to plan for their arrival in terms of housing and infrastructure</p>' + 
...
function totalPopulationMap() {
	rScale.domain([d3.min(data, function(d) {return d["pop_2001"];}),d3.max(data, function(d) {return d["pop_2011"];})])
		  .range([1,20]);
	rVar = "pop"
	changeMap();
}//totalPopulationMap
...
function totalUrbanLandMap() {
	rScale.domain([d3.min(data, function(d) {return d["land_2011"];}),d3.max(data, function(d) {return d["land_2011"];})])
		  .range([1,20]);
	rVar = "land"
	changeMap();	
}//totalUrbanLandMap
...
function totalDensityMap() {
	rScale.domain([d3.min(data, function(d) {return d["density_2001"];}),d3.max(data, function(d) {return d.density_2011;})])
		  .range([1,8]);
	rVar = "density"
	changeMap();	
}//totalDensityMap
...
function changeMap() {
	...
	cities2010.selectAll(".city_2010")
		...
		.transition().duration(1500)
			.attr("r", function(d) {return eval("rScale(d." + rVar + "_2011)");})
	...
	cities2000.selectAll(".city_2000")
		...
		.on("mouseout", showAll)
		.transition().duration(1500)
			.attr("r", function(d) {return eval("rScale(d." + rVar + "_2001)");})
