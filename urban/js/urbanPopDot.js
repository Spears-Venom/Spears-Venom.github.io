...
function callScatter() {
	...
	var bar = barChart.selectAll("g")
		.data(countries)
		.enter().append("g")
		.filter(function(d) {return d.State != "Kerala" & d.gdp_2001 != 0;})
	...
	bar.append("rect")
		...
		.attr("transform", function (d) {
			return "translate(" + (barScale(d.pop_perc_2011) - dotSize/2) + "," + (yBarScale(d.gdp_2011) - dotSize/2) + ")";
		});
	...
	bar.append("rect")
		...
		.attr("height",dotSize)
		.attr("transform", function (d) {
			return "translate(" + (barScale(d.pop_perc_2001) - dotSize/2) + "," + (yBarScale(d.gdp_2001) - dotSize/2) + ")";
		});
...
function createScatter() {
	...
	barChart.selectAll("line")
		...
		.attr("y1", function(d) {return yBarScale(d.gdp_2001);})
		.attr("y2", function(d) {return yBarScale(d.gdp_2011);});
